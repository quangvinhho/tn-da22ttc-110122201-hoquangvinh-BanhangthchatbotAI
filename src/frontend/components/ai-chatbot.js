...
(function() {
  ...
  function createChatbotHTML() {
    const chatbotHTML = `
      ...
      <button class="ai-chatbot-btn" id="ai-chatbot-btn" title="Chat với AI">
        <img src="images/chatbot AI.jpg" alt="QuangHưng AI">
      </button>
      
      <!-- Bong bóng chào hỏi ngoài (Tooltip) -->
      <div class="ai-chatbot-tooltip" id="ai-chatbot-tooltip" style="display: none;">
        <div class="ai-tooltip-text">
          <strong>QuangHưng AI</strong> sẵn sàng tư vấn và hỗ trợ bạn! 👋
        </div>
        <button class="ai-tooltip-close" id="ai-tooltip-close" title="Đóng">&times;</button>
      </div>
        ...
        <div class="ai-chat-main">
          <div class="ai-chat-header">
            ...
            <div class="ai-chat-info">
              <h3 class="ai-chat-title">QuangHưng AI</h3>
  ...
  function startNewConversation() {
    ...
    if (userId) {
      ...
      addBotMessage(`Xin chào ${userName}! 👋\n\nTôi là QuangHưng, trợ lý AI của QuangHưng Mobile. Bạn cần hỗ trợ gì?`);
    } else {
  ...
  async function loadInitialChat() {
    ...
    if (conversations.length > 0) {
      ...
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      const userName = user.ho_ten || 'bạn';
      addBotMessage(`Xin chào ${userName}! 👋\n\nTôi là QuangHưng, trợ lý AI của QuangHưng Mobile. Lịch sử chat của bạn sẽ được lưu lại.\n\nBạn cần hỗ trợ gì?`);
      renderSuggestions(DEFAULT_SUGGESTIONS);
    }
  }
  ...
  function initChatbot() {
    ...
    const input = document.getElementById('ai-chat-input');
    const tooltip = document.getElementById('ai-chatbot-tooltip');
    const tooltipClose = document.getElementById('ai-tooltip-close');

    // Quản lý hiển thị bong bóng chào hỏi
    function showTooltip() {
      if (tooltip && sessionStorage.getItem('ai-tooltip-dismissed') !== 'true' && !chatWindow.classList.contains('active')) {
    ...
    if (tooltipClose) {
      tooltipClose.addEventListener('click', (e) => {
        e.stopPropagation();
        hideTooltip();
        sessionStorage.setItem('ai-tooltip-dismissed', 'true');
      });
    }

    // Hiển thị bong bóng sau 2 giây
